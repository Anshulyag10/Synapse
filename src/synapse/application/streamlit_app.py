"""Streamlit frontend for Synapse.
...
from synapse.application.cli import SynapseEngine
...
RULES = [
    ...
    (r"Initialising Synapse", "Starting Synapse"),
...
st.markdown(
    '<div id="eg-head"><span class="dot"></span><h1>Synapse</h1></div>'
...
if "engine" not in st.session_state:
    with st.status("Starting Synapse…", expanded=True) as boot:
        try:
            st.session_state.engine = get_engine(lambda step: st.write(f"• {step}"))
            boot.update(label="Synapse is ready", state="complete", expanded=False)
        except Exception:
            boot.update(label="Could not start Synapse", state="error")
            st.error(
                "Synapse could not connect to its data stores. "
