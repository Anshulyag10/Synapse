"""Structured logging for EvidenceGraph.
...
def setup_logging(
    ...
    component: str = "evidence_graph",
) -> logging.Logger:
    """Configure the root EvidenceGraph logger with console + optional file output.
    ...
    component
        Logger name (default ``evidence_graph``).
...
def get_logger(name: str = "evidence_graph") -> logging.Logger:
    """Return a child logger (creates the root logger lazily if needed)."""
    root = logging.getLogger("evidence_graph")
